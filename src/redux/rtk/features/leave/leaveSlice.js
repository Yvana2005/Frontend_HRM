	...
	async (id) => {
		try {
			const { data } = await axios({
				...
				url: `leave-application/${id}/leaveHistory`,
			});
			// const { data } = await axios.get(``);
			return data;
		} catch (error) {
			toast.error("Error in getting leavehistory list try again");
			console.log(error.message);
			return {
				message: "error",
			};
...
const LeaveApplicationSlice = createSlice({
	...
	reducers: {
		...
		clearLeaveApplicationList: (state) => {
			state.list = null;
		},
	},
...
export const { clearLeaveApplication, clearLeaveApplicationList } = LeaveApplicationSlice.actions;
